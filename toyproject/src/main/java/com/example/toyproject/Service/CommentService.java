...
public class CommentService {
	...
	public List<Comment> commentListService(long boardNo) throws Exception {
		Board board = BR.findById(boardNo).get();
		List<Comment> commentlist = CR.findByBoard(board);
		return commentlist;
	}

	public int commentInsertService(Comment comment, long boardNo) throws Exception {
		int result = 1;
		Board board = BR.findById(boardNo).get();
		...
		return commentInsert(comment, result, boardNo);
	}

	public int commentUpdateService(int commentPrimeNo, String content) throws Exception {
	...
	public int commentDeleteService(int commentPrimeNo) throws Exception {
		Comment comment = CR.findById(commentPrimeNo).get();
		int result = 0;
		if (comment.getUserid().equals(session.getAttribute("userid"))) {
	...
	public int commentInsert(Comment comment, int result, long boardNo) {
